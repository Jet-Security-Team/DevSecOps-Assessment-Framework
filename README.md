# DevSecOps Assessment Framework (DAF) [](#){name=daf_heading}

### Cодержание

- [Введение](#intro)
    - [Важный дисклеймер](#important_disclaimer)
- [Цели и задачи DAF](#purposes_objectives)
- [Описание DAF](#description)
    - [Карта DAF (ex - *Пиратская карта*)](#map)
        - [Модель Технологий](#map_model_technology)
        - [Модель Процессов](#map_model_processes)
    - [Таблица оценки и Тепловая матрица](#eval_table_and_therm_matrix)
        - [Таблица оценки](#eval_table)
        - [Тепловая матрица (Heatmap)](#therm_matrix)
    - [Пирамида зрелости (ex - *Кирилламида*)](#pyramid_maturity)
- [Как пользоваться фреймворком](#how_to_usage)
- [Материалы, используемые при создании](#additional_materials)
...
- Таблица оценки и Тепловая матрица зрелости;
...
## Цели и задачи DAF [](#){name=purposes_objectives}
...
*Карта DAF* — это верхнеуровневый взгляд на весь фреймворк. Она включает в себя все аспекты процесса безопасной разработки с этапа планирования до перевода ПО в промышленную эксплуатацию. Карта делится на два блока: модель технологий и процессов.

#### Модель *Технологии* [](#){name=map_model_technology}

![new_model_technology](./images/Model_Technology.png)

#### Модель *Процессы* [](#){name=map_model_processes}

![new_model_processes](./images/Model_Processes.png)

### Таблица оценки и Тепловая матрица [](#){name=eval_table_and_therm_matrix}

#### Таблица оценки

Таблица оценки содержит различные практики, а также критерии оценки (”Верно” и “Неверно” для нулевого этапа, а также “Выполняется”, “Частично выполняется” и “Не выполняется” для практик 1го и последующих этапов зрелости). В таблице оценки практики сгруппированы в поддомены, а поддомены - в домены. Для соответствия  конкретному этапу зрелости может потребоваться выполнение одной или нескольких практик.
...
### Пирамида зрелости [](#){name=pyramid_maturity}
...
## Как пользоваться фреймворком [](#){name=how_to_usage}
...
## Материалы, используемые при создании [](#){name=additional_materials}
...
## Связаться с нами [](#){name=contacts}
